  ...
  return (
    <aside className="w-64 flex-shrink-0 bg-white border-r border-gray-200 p-4 flex flex-col">
      <nav className="flex flex-col gap-1 text-sm font-normal">
        ...
        <div className="px-3 pt-4 pb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">SIS</div>
        ...
        {hasAnyScope(['school:admin']) && <NavItem to={`/school/${siteId}/homework/reports`} icon={<ChartBarIcon className="h-4 w-4" />} label="Homework Reports" />}
        ...
        {hasAnyScope(['school:admin']) && <NavItem to={`/school/${siteId}/admin/roles`} icon={<CogIcon className="h-4 w-4" />} label="Roles & Permissions" />}
